...
def run(request: dict) -> dict:
    ...
    if not target.is_file() or not isinstance(assets, list) or not assets:
        return {"ok": False, "category": "CLOTHING_REQUEST", "error": "A target rig and at least one clothing asset are required"}
    for asset in assets:
        if not Path(str(asset.get("asset_path", ""))).expanduser().resolve().is_file():
            return {"ok": False, "category": "INPUT_MISSING", "error": f"Clothing asset does not exist: {asset.get('asset_path')}"}
    output.parent.mkdir(parents=True, exist_ok=True)
    descriptor, name = tempfile.mkstemp(prefix="character-factory-clothing-", suffix=".json", dir=output.parent)
    os.close(descriptor)
    ...
    try:
        ...
        script = Path(__file__).resolve().parents[2] / "tools" / "blender" / "transfer_clothing.py"
    ...
    if result_line is None:
        return {"ok": False, "category": "CLOTHING_PROTOCOL", "error": "Blender clothing script did not return a result marker", "stdout": completed.stdout, "stderr": completed.stderr}
    result = json.loads(result_line[len(marker):])
    if not result.get("ok", False):
        return {**result, "stdout": completed.stdout, "stderr": completed.stderr}
    if not output.is_file() or output.stat().st_size == 0:
        return {"ok": False, "category": "CLOTHING_OUTPUT_INVALID", "error": f"Blender did not produce clothing output: {output}", "stdout": completed.stdout, "stderr": completed.stderr}
