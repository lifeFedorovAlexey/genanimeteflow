...
def run(request: dict) -> dict:
    ...
    if not source.is_file():
        return {"ok": False, "category": "INPUT_MISSING", "error": f"Source mesh does not exist: {source}"}
    output.parent.mkdir(parents=True, exist_ok=True)
    descriptor, name = tempfile.mkstemp(prefix="character-factory-ik-", suffix=".json", dir=output.parent)
    os.close(descriptor)
    ...
    try:
        ...
        script = Path(__file__).resolve().parents[2] / "tools" / "blender" / "setup_ik.py"
    ...
    if result_line is None:
        return {"ok": False, "category": "IK_PROTOCOL", "error": "Blender IK script did not return a result marker", "stdout": completed.stdout, "stderr": completed.stderr}
    result = json.loads(result_line[len(marker):])
    if not result.get("ok", False):
        return {**result, "stdout": completed.stdout, "stderr": completed.stderr}
    if not output.is_file() or output.stat().st_size == 0:
        return {"ok": False, "category": "IK_OUTPUT_INVALID", "error": f"Blender did not produce a non-empty IK output: {output}", "stdout": completed.stdout, "stderr": completed.stderr}
