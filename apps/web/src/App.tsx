...
function nextStep(job: Job, capabilities?: Capabilities): { title: string; body: string; stage?: string; action?: string; tone: "action" | "blocked" | "done" } {
  ...
  const pending = stageOrder.find(stage => (stage !== "equipment" || job.equipment_assets.length > 0 || job.stages.equipment?.status !== "PENDING") && (stage !== "clothing" || job.clothing_assets.length > 0 || job.stages.clothing?.status !== "PENDING"));
  if (pending) {
