...
import { api, Capabilities, Hardware, Job, MotionCatalog, StageRecord, ViewName } from "./api";
...
function App() {
  ...
  const refresh = useCallback(async () => { try { const [hw, caps, list, motions] = await Promise.all([api.hardware(), api.capabilities(), api.jobs(), api.motions()]); setHardware(hw); setCapabilities(caps); setMotionCatalog(motions); setJobs(list); setActive(current => current ? list.find(job => job.job_id === current.job_id) ?? current : list[0]); } catch (err) { setError(err instanceof Error ? err.message : String(err)); } }, []);
