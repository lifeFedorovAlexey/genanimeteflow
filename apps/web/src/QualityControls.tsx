...
export default function QualityControls({ job, busy, onSave }: Props) {
  ...
  const payload = {
    profile,
    resolution: Number(resolution) as 384 | 512 | 640 | 768,
    ...
    texture_resolution: Number(textureResolution) as 512 | 1024,
  };
  ...
  return <details className="quality-settings">
    ...
    <div className="quality-fields">
      ...
      <label>Размер входа<select value={resolution} onChange={event => setResolution(event.target.value)}><option value="384">384 px</option><option value="512">512 px</option><option value="640">640 px</option><option value="768">768 px</option></select></label>
      ...
      {profile === "CUSTOM" && <>
        ...
        <label>Texture<select value={textureResolution} onChange={event => setTextureResolution(event.target.value)}><option value="512">512 px</option><option value="1024">1024 px</option></select></label>
    ...
    <p className="quality-hint">Профиль меняет реальные параметры worker; изменение размера входа повторно обработает референсы. После смены provider/качества нужные этапы будут помечены «изменён».</p>
  </details>;
}
