...
type TesterStatus = { state: string; clip: string; speed: number; time: number; rootMotion: boolean };
...
function AnimatedAsset({ url, selectedClip, wireframe, skeleton, onActions, onStatus }: { url: string; selectedClip: string; wireframe: boolean; skeleton: boolean; onActions: (names: string[]) => void; onStatus: (status: TesterStatus) => void }) {
  ...
  useFrame((_, delta) => {
    ...
    const clip = findAction(actionNames, selected.state, selectedClip);
    ...
    if (statusKey !== lastStatus.current) { lastStatus.current = statusKey; onStatus({ state: selected.state, clip, speed: selected.speed, time: action?.time ?? 0, rootMotion: false }); }
  });
...
export default function Viewport({ assetUrl }: { assetUrl?: string }) {
  ...
  const [status, setStatus] = useState<TesterStatus>({ state: "idle", clip: "", speed: 1, time: 0, rootMotion: false });
  const handleActions = (names: string[]) => { setActions(names); setSelectedClip(current => current && names.includes(current) ? current : names[0] ?? ""); };
  return <div className="viewport-shell"><div className="viewport"><Canvas camera={{ position: [3, 2.2, 4], fov: 42 }}><color attach="background" args={["#0c0e12"]} /><ambientLight intensity={1.2} /><directionalLight position={[3, 5, 2]} intensity={2} /><Grid args={[10, 10]} cellColor="#29303a" sectionColor="#4a5666" fadeDistance={12} />{assetUrl && <AnimatedAsset url={assetUrl} selectedClip={selectedClip} wireframe={wireframe} skeleton={skeleton} onActions={handleActions} onStatus={setStatus} />}<OrbitControls makeDefault /></Canvas><div className={`viewport-empty ${assetUrl ? "has-asset" : ""}`}><span>{assetUrl ? "Validated asset" : "Unit Tester"}</span><small>{assetUrl ? "WASD move · Shift sprint · Ctrl crouch · Space jump" : "Validated GLB assets will appear here"}</small></div></div>{assetUrl && <div className="tester-toolbar"><label>Clip<select value={selectedClip} onChange={event => setSelectedClip(event.target.value)} disabled={!actions.length}><option value="">Auto state</option>{actions.map(name => <option key={name} value={name}>{name}</option>)}</select></label><label className="debug-toggle"><input type="checkbox" checked={wireframe} onChange={event => setWireframe(event.target.checked)} /> Wireframe</label><label className="debug-toggle"><input type="checkbox" checked={skeleton} onChange={event => setSkeleton(event.target.checked)} /> Skeleton</label><div className="tester-stats"><span>{status.state}</span><span>{status.clip || "no clip"}</span><span>{status.speed.toFixed(1)}×</span><span>{status.rootMotion ? "root motion" : "in-place"}</span></div></div>}</div>;
}
