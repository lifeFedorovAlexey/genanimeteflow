...
function AnimatedAsset({ url, selectedClip, upperBodyAction, graphClip, graphBlend, wireframe, skeleton, renderMode, onActions, onStatus }: { url: string; selectedClip: string; upperBodyAction: string; graphClip: string; graphBlend: GraphBlend[]; wireframe: boolean; skeleton: boolean; renderMode: RenderMode; onActions: (names: string[]) => void; onStatus: (status: TesterStatus) => void }) {
  ...
  useFrame((_, delta) => {
    ...
    const statusKey = `${selected.state}|${clip}|${upperClip}|${selected.speed}`;
...
export default function Viewport({ assetUrl, jobId, graphEnabled, equipmentType }: { assetUrl?: string; jobId?: string; graphEnabled?: boolean; equipmentType?: string | null }) {
  ...
  const handleActions = useCallback((names: string[]) => { setActions(names); setSelectedClip(current => current && names.includes(current) ? current : ""); setUpperBodyAction(current => current && names.includes(current) ? current : ""); }, []);
